- When adding/removing crates from the project, ALWAYS use cargo commands to do so, don't edit Cargo.toml directly.
- Never pipe to 2>/dev/null.
- Don't use cat to create files, use create_file.
...
- Options can now be placed anywhere after the subcommand: `ripdoc print --offline tokio` or `ripdoc print tokio --offline`
